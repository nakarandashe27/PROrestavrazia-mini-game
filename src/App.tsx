...
import { MosaicHero, GameTitle, Partners } from './components/MosaicHero';
import { StandStats } from './components/StandStats';
import { initAnalytics, track, METRIKA_ID } from './analytics';
...
export default function App(){
 ...
 const [statsOpen,setStatsOpen]=useState(()=>new URLSearchParams(location.search).has('stats'));
 const [restoration,setRestoration]=useState({done:0,total:0});
 ...
 useEffect(()=>{initAnalytics(/[?&]telegram=|tgWebAppPlatform/.test(location.search+location.hash)?'telegram':'web')},[]);
 ...
 function start(p:1|2=1){track('game_start',{route:p===2?'workshop':route});unlockSound();if(sound)chime(true);const sd=newSeed();setSeed(sd);setPhase(p);setTermIdx(0);setCount(0);setRestored([]);setFieldNotes([]);setField(null);setTermsDone(0);setSeconds(0);setToast('');engine.current!.time=0;engine.current!.restored=0;if(lighting==='cycle')engine.current!.nightAmount=0;setScreen('intro')}
 function nextTerm(){if(termIdx+1<totalTerms)load(1,termIdx+1);else{setPhase(2);setScreen('intro')}}
 function finishDetail(){const result=[...restored,current!.id];setRestored(result);engine.current!.restored=result.length*6/totalParts;chime(sound);if(result.length===totalParts){setSeconds(Math.floor(engine.current!.time));setToast('');track('game_finish',{route});setScreen('end')}else{setToast(`${current!.name} — сохранено в альбоме`);setScreen('play')}}
 ...
 return <div className="app" data-screen={screen}>
  ...
  {screen==='start'&&<main className="welcome mosaic-mode">
   <MosaicHero lighting={lighting} onRestore={(done,total)=>setRestoration({done,total})}/><div className="mosaic-shade"/>
   <div className="hero-copy"><Partners/><GameTitle/><div className="eyebrow">По следам типографии Сытина · 1903</div><p>Соберите слово, поднимитесь на леса у фасада и попробуйте себя в роли реставратора.</p>
   ...
   {restoration.total>0&&<div className={'mosaic-hint'+(restoration.done>=restoration.total?' done':'')} aria-live="polite">{restoration.done>=restoration.total?<><b>✓</b><span>Фасад восстановлен. Спасибо, реставратор!<small>Теперь — в путешествие по типографии.</small></span></>:<><b>{Math.round(restoration.done/restoration.total*100)}%</b><span><span className="hint-mouse">Проведите курсором по фасаду</span><span className="hint-touch">Коснитесь фасада</span> — верните утраченную смальту<small>Мозаика откликается на движение, клик пускает волну</small></span></>}</div>}
  ...
  {screen==='start'&&<GameVersion onStats={()=>setStatsOpen(true)}/>}
  {statsOpen&&screen==='start'&&<StandStats onClose={()=>setStatsOpen(false)}/>}
  {screen==='play'&&<>
